...
func (a *App) printUsage() {
	...
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Setup command usage:     cms setup [task] [--force] [--arguments=<argument_value>]")
	fmt.Fprintln(w, "")
	...
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "    root_ca                   Create its own self signed Root CA keypair in /etc/cms/root-ca/ for quality of life")
	fmt.Fprintln(w, "                              - Option [--force] overwrites any existing files, and always generate new Root CA keypair")
	fmt.Fprintln(w, "                              Optional env variables:")
	fmt.Fprintln(w, "                                  - CMS_CA_CERT_VALIDITY=<cert life span>     : Certificate Management Service Root Certificate Validity")
	...
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "    intermediate_ca           Create its own root_ca signed intermediate CA keypair(signing, tls-server and tls-client) in /etc/cms/intermediate-ca/ for quality of life")
	fmt.Fprintln(w, "                              - Option [--force] overwrites any existing files, and always generate new root_ca signed Intermediate CA keypair")
	fmt.Fprintln(w, "                              Available argument is:")
	fmt.Fprintln(w, "                                  - type          available options are: TLS, TLS-Client, Signing")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "    tls                       Create its own intermediate_ca signed TLS keypair in /etc/cms for quality of life")
	fmt.Fprintln(w, "                              - Option [--force] overwrites any existing files, and always generate intermediate_ca signed TLS keypair")
	fmt.Fprintln(w, "                              Available argument and optional env variable is:")
	...
	fmt.Fprintln(w, "                              Available argument and optional env variables are:")
	...
	fmt.Fprintln(w, "                              Optional env variables:")
	fmt.Fprintln(w, "                                  - CMS_SERVER_READ_TIMEOUT=<read timeout>                    : Certificate Management Service Read Timeout")
	fmt.Fprintln(w, "                                  - CMS_SERVER_READ_HEADER_TIMEOUT=<read header timeout>      : Certificate Management Service Read Header Timeout")
	fmt.Fprintln(w, "                                  - CMS_SERVER_WRITE_TIMEOUT=<write timeout>                  : Certificate Management Service Write Timeout")
	fmt.Fprintln(w, "                                  - CMS_SERVER_IDLE_TIMEOUT=<idle timeout>                    : Certificate Management Service Idle Timeout")
